...
export function runVerificationCommand(cwd: string, command: string): string {
  ...
  const output = redactSecrets([`$ ${command}`, result.stdout ?? '', result.stderr ?? ''].join('\n')).trim();
...
export function parseCiLog(cwd: string, logPath: string): string {
  ...
  return summarizeFailure(raw, /(?:failed|error|exit code [1-9])/i.test(raw) ? 1 : 0);
}

export function summarizeFailure(output: string, status: number): string {
  const interesting = output
    .split('\n')
    .filter(line => /error|fail|failed|✗|×|TS\d+|ERR!|AssertionError|TypeError|ReferenceError|SyntaxError/i.test(line))
    .slice(0, 80)
    .join('\n');
  return [
    `Status: ${status}`,
    interesting ? `Relevant output:\n${interesting}` : `Output:\n${output.slice(0, 4000)}`,
  ].join('\n');
}
