...
export function classifyCommand(command: string): RiskLevel {
  ...
  if (DANGEROUS_COMMANDS.some(pattern => pattern.test(trimmed))) return 'high';
  if (SAFE_COMMANDS.some(pattern => pattern.test(trimmed))) return 'low';
  if (/^(npm|pnpm|yarn)\s+(install|add|remove|update)\b/.test(trimmed)) return 'high';
  if (/^git\s+(add|commit|reset|checkout|clean|push|pull|merge|rebase)\b/.test(trimmed)) return 'high';
  return 'medium';
}

export function decidePolicy(profile: PolicyProfile, action: 'read' | 'write' | 'delete' | 'command', detail = '', allowRules: AllowRuleStore = { commands: [] }): PolicyDecision {
