...
export class TaskSession {
  ...
  constructor(opts: { cwd: string; task: string; model: string; policy?: PolicyProfile }) {
    ...
    this.writeSummary();
    this.record('run_started', { task: opts.task, model: opts.model, policy: this.policy });
    gcRuns(this.cwd, 50);
  }

  record(type: string, data: Record<string, unknown> = {}): void {
    const event = redactSecrets(JSON.stringify({ type, timestamp: new Date().toISOString(), ...data }));
    fs.appendFileSync(path.join(this.runDir, 'events.jsonl'), `${event}\n`, 'utf-8');
  }
  ...
  captureBefore(file: string): void {
    ...
    if (beforeExists) {
      beforeSnapshot = path.join('snapshots', `${crypto.randomBytes(6).toString('hex')}.before`);
  ...
  noteChange(file: string): void {
    ...
    this.summary.modifiedFiles = Array.from(this.changedFiles.keys()).map(f => this.guard.relative(f));
    this.writeChanges();
    this.writeSummary();
    this.record('file_changed', { file: this.guard.relative(resolved), before: existing.beforeHash, after });
  }
  ...
  canMutate(file: string): { ok: boolean; reason?: string } {
    ...
    if (readHash === undefined) {
      return { ok: false, reason: `Refusing to modify unread file: ${this.guard.relative(resolved)}` };
    }
    const currentHash = hashFile(resolved);
    if (readHash !== currentHash) {
      return { ok: false, reason: `Refusing stale edit; file changed since read: ${this.guard.relative(resolved)}` };
    }
    return { ok: true };
  }
  ...
  private writeSummary(): void {
    fs.writeFileSync(path.join(this.runDir, 'run.json'), JSON.stringify(this.summary, null, 2) + '\n', 'utf-8');
  }
...
export function gcRuns(cwd: string, keepLimit = 50): void {
  try {
    ...
    console.error('[TaskSession] Warning: failed to garbage collect old runs', err);
  }
}
...
export function undoRun(cwd: string, id: string): string {
  ...
  if (!fs.existsSync(changesFile)) return `Undo refused: run ${id} has no change ledger.`;
  const guard = new WorkspaceGuard(cwd);
  const changes = JSON.parse(fs.readFileSync(changesFile, 'utf-8')) as ChangeRecord[];
  ...
  for (const change of changes) {
    ...
    if (currentHash !== change.afterHash) {
      conflicts.push(`${change.file} changed after run (expected ${change.afterHash ?? 'missing'}, got ${currentHash ?? 'missing'})`);
    }
  }
  ...
  for (const change of changes.slice().reverse()) {
    ...
    if (change.beforeExists) {
      if (!change.beforeSnapshot) return `Undo failed: missing snapshot for ${change.file}`;
...
export function revertAgentCommit(cwd: string): string {
  try {
    const message = execFileSync('git', ['log', '-1', '--format=%s'], { cwd, encoding: 'utf-8' }).trim();
    if (!message.includes('[fixo-run:')) {
      return 'Undo refused: last commit is not marked as a FixO-owned commit.';
    }
    execFileSync('git', ['revert', '--no-edit', 'HEAD'], { cwd, encoding: 'utf-8' });
