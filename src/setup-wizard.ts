...
async function runDirectSetup(): Promise<FreeLLMConfig> {
  ...
  const { ProvidersManager, PROVIDER_REGISTRY } =
  ...
  while (true) {
    ...
    const s = p.spinner();
    s.start(
      `Verifying connection and fetching models for ${def.displayName}...`,
    );

    try {
      fetchedModels = await ProvidersManager.verifyKeyAndFetchModels(
        providerName as string,
        apiKey as string,
      );
      s.stop(
        `✓ Verification successful. Found ${fetchedModels.length} models.`,
      );
      break;
    } catch (err: any) {
  ...
  const initialModel =
    fetchedModels.find((m) => def.models.includes(m)) || fetchedModels[0];
...
└────────────────────────────────────────────────────────────────┘\n`);
  ...
  const serverChoice = await p.select({
    ...
    options: [
      { value: DEFAULT_API_URL, label: "Cloud Hosted SaaS (Default)" },
      {
        ...
        label: "Local Development Server (http://localhost:3001/v1)",
      },
  ...
  const configureProviders = await p.confirm({
    message:
      "Also add direct provider keys? (You can do this later via /providers add)",
    initialValue: false,
  });
  ...
  if (configureProviders) {
    ...
    if (!p.isCancel(selectedProviders) && selectedProviders.length > 0) {
      ...
      if (configuredCount > 0) {
        p.outro(
          `✓ ${configuredCount} provider key(s) saved to ~/.fixocli/providers.json`,
        );
      }
    }
  }
