...
export function renderSessionHeader(opts: SessionHeaderOptions): void {
  ...
  try {
    ...
    process.stdout.write(
      `  ${C.SNOW4}│${C.RESET}  ${padInside(line1, inner)}${C.SNOW4}│${C.RESET}\n`,
    );
    process.stdout.write(
      `  ${C.SNOW4}│${C.RESET}  ${padInside(line2, inner)}${C.SNOW4}│${C.RESET}\n`,
    );
    process.stdout.write(
      `  ${C.SNOW4}│${C.RESET}  ${padInside(line3, inner)}${C.SNOW4}│${C.RESET}\n`,
    );
    if (endpointLine) {
      process.stdout.write(
        `  ${C.SNOW4}│${C.RESET}  ${padInside(endpointLine, inner)}${C.SNOW4}│${C.RESET}\n`,
      );
    }
