 ...
 */
export function lava(text: string): string   { return `${C.LAVA}${text}${C.RESET}`; }
export function snow(text: string): string   { return `${C.SNOW}${text}${C.RESET}`; }
export function dim(text: string): string    { return `${C.SNOW4}${text}${C.RESET}`; }
...
export function purple(text: string): string { return `${C.PURPLE}${text}${C.RESET}`; }
...
export function providerColor(name: string): string {
  ...
  if (lower.includes('anthropic') || lower.includes('claude')) return C.P_ANTHROPIC;
