import { C, padToVisual, visLen } from './colors.js';
import { safeWrite, safeWriteLine } from './render-primitives.js';

export interface LoadingPhase {
  id: 'routing' | 'reasoning' | 'reading' | 'executing' | 'writing' | 'verifying' | 'searching' | 'completed';
  label: string;
  detail?: string;
  icon: string;
}
...
export class LoadingAnimation {
  private phase: LoadingPhase = { id: 'reasoning', label: 'Reasoning…', icon: '⚡' };
  ...
  start(): void {
    ...
    if (this.isTTY) {
      ...
      // Non-TTY fallback
      safeWriteLine(`  ${this.phase.icon} ${this.phase.label} ${this.phase.detail ? `· ${this.phase.detail}` : ''}`);
    }
  }
  ...
  private draw(): void {
    ...
    const detail = this.phase.detail ? `  ${C.SNOW4}${this.phase.detail}${C.RESET}` : '';
    ...
    let hintLine = '';
    if (elapsedMs > 3000) {
      this.hintIndex = Math.floor((elapsedMs - 3000) / 8000) % this.hints.length;
