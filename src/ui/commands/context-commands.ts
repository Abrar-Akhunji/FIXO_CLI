...
export const todoCommand: CommandHandler = async (ctx) => {
    ...
    if (sub === 'clear') {
      ...
      const result = saveTodoList(ctx.cwd, { ...list, items: kept, updatedAt: Date.now() });
      if (!result.ok) {
        console.log(`\n${colors.red}✗ Failed to save todo: ${result.error}${colors.reset}`);
        return;
      }
...
export const mcpCommand: CommandHandler = async (ctx) => {
    ...
    if (!sub || sub === 'list') {
      ...
      const renderSource = (label: string, s: { configPath: string | null; servers: Record<string, unknown> }) => {
        ...
        if (names.length === 0) {
          console.log(`  ${colors.dim}${label}: (empty)${s.configPath ? ` ${colors.dim}${s.configPath}${colors.reset}` : ''}`);
          return;
        }
        console.log(`  ${colors.bold}${label}${colors.reset}${s.configPath ? ` ${colors.dim}${s.configPath}${colors.reset}` : ''}`);
    ...
    if (sub === 'add') {
      ...
      addLocalMcpServer(ctx.cwd, name, { command: cmd, args: cmdArgs, type: 'stdio' });
...
export const compactCommand: CommandHandler = async (ctx) => {
    ...
    console.log(`${colors.dim}  Current context: ${(tokensBefore / 1000).toFixed(0)}k / ${(contextLimit / 1000).toFixed(0)}k tokens${colors.reset}`);
    try {
      const compacted = await ctx.conversation.compact(ctx.agent.getClient(), ctx.state.currentModel);
      if (compacted) {
        const info = ctx.conversation.getLastCompactionInfo();
        const tokensAfter = ctx.conversation.getTotalTokens();
        console.log(`${colors.green}✓ Compacted: ${info?.messagesBefore ?? msgCount} messages → summary + ${ctx.conversation.getMessageCount()} recent messages.${colors.reset}`);
        console.log(`${colors.dim}  Context: ${(tokensBefore / 1000).toFixed(0)}k → ${(tokensAfter / 1000).toFixed(0)}k tokens (~${((info?.tokensFreed ?? 0) / 1000).toFixed(0)}k freed).${colors.reset}`);
      } else {
        console.log(`${colors.dim}Not enough messages to compact (need more than 4 messages).${colors.reset}`);
      }
    } catch (err: any) {
...
export const variantCommand: CommandHandler = async (ctx) => {
