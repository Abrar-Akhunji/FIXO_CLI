...
export const fixTestsCommand: CommandHandler = async (ctx) => {
  ...
  while (attempt <= maxAttempts) {
    ...
    try {
      ...
      const result = await ctx.agent.runStreaming(
        context,
        ctx.conversation,
        ctx.rl,
      );
...
export const fixCiCommand: CommandHandler = async () => {
  console.log(
...
export const runPlanCommand: CommandHandler = async (ctx) => {
  ...
  if (fs.existsSync(dagFile)) {
    try {
      ...
      const maxAttempts = ctx.projectConfig?.maxAttempts ?? 3;
      ...
      if (success) {
        ...
        if (ctx.git.isGitRepo()) {
          ...
          const { getModifiedFiles, getBranchPoint } =
            await import("../../agent/worker-agent.js");
