...
import { getDefaultConfig, loadConfig, saveConfig, getConfigPath } from '../config.js';
...
test('loadConfig — back-compat: pre-v1.1 config with FreeLLMAPI key is inferred as proxy', () => {
  ...
  try {
    ...
    assert.equal(cfg.provider_mode, 'proxy', 'must NOT silently flip a working proxy user to direct');
...
test('save then load — direct-mode config round-trips through disk', () => {
  ...
  try {
    ...
    toSave.directProvider = { name: 'anthropic', defaultModel: 'claude-opus-4-7' };
    toSave.defaultModel = 'claude-opus-4-7';
    toSave._firstRunComplete = true;
    saveConfig(toSave);

    assert.ok(fs.existsSync(getConfigPath()), 'config file should exist after save');
    ...
    assert.equal(loaded.freellmapi_api_key, undefined, 'direct mode should not persist a proxy key');
  } finally {
    ctx.restore();
  }
});
