...
test('defaultIsRetryable recognises the standard retryable statuses', () => {
  for (const status of DEFAULT_RETRYABLE_STATUS_CODES) {
    assert.equal(defaultIsRetryable(new HttpError(status, 'x')), true, `status ${status}`);
  }
...
test('defaultIsRetryable recognises network-layer error messages', () => {
  for (const msg of ['ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT', 'fetch failed']) {
...
test('withRetry retries until the policy is exhausted, then throws the last error', async () => {
  ...
  await assert.rejects(
    withRetry(
      ...
      { ...DEFAULT_RETRY_POLICY, maxAttempts: 3, baseDelayMs: 1, maxDelayMs: 1, jitter: 'none' },
    ),
...
test('withRetry calls the onRetry hook with the correct fields', async () => {
  const events: Array<{ attempt: number; retryAfterMs: number | null; delayMs: number }> = [];
  let calls = 0;
  await assert.rejects(
    withRetry(
      async () => {
      ...
      },
      {
        ...
        jitter: 'none',
        onRetry: (info) => {
          events.push({ attempt: info.attempt, retryAfterMs: info.retryAfterMs, delayMs: info.delayMs });
        },
      },
    ),
  );
...
test('withRetry surfaces Retry-After from the error headers property', async () => {
  ...
  await assert.rejects(
    withRetry(
      async () => {
        calls += 1;
        const err = new HttpError(429, 'rate limited') as HttpError & { headers?: Record<string, string> };
        err.headers = { 'Retry-After': '0' };
        throw err;
      },
      { ...DEFAULT_RETRY_POLICY, maxAttempts: 2, baseDelayMs: 100_000, maxDelayMs: 100_000, jitter: 'none' },
    ),
  );
