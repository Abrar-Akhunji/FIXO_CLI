...
test('fingerprintToolResult — tail truncation is stable', () => {
  const d = new LoopTrapDetector({ ...DEFAULT_LOOP_TRAP_PREFS, toolResultTailBytes: 64 });
  ...
  assert.equal(
    d.fingerprintToolResult(long),
    d.fingerprintToolResult(shorter),
  );
});
...
test('record — three equivalent turns return trap-detected with layers', () => {
  ...
  if (v.state === 'trap-detected') {
    ...
    assert.deepEqual([...v.layers].sort(), ['tool-args', 'tool-result', 'workspace']);
