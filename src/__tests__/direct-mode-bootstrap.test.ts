...
import { AgentClient, DirectModelUnresolvedError } from '../agent/agent-client.js';
...
test('direct mode — chat() routes to OpenAI base URL, never to FreeLLMAPI proxy', async () => {
  ...
  try {
    ...
    const result = await client.chat([{ role: 'user', content: 'hi' }], 'gpt-4o');
...
test('direct mode — unknown model throws DirectModelUnresolvedError instead of falling through to proxy', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => client.chat([{ role: 'user', content: 'hi' }], 'totally-unknown-model-xyz-9999'),
      (err: unknown) => {
        assert.ok(err instanceof DirectModelUnresolvedError, `expected DirectModelUnresolvedError, got ${err}`);
        assert.match(
    ...
    assert.equal(fetchCalled, false, 'no HTTP call must be issued when direct-mode resolution fails');
  } finally {
    ctx.restore();
  }
});
...
test('direct mode — chatStream() also refuses to leak to proxy on unresolved model', async () => {
  ...
  try {
    ...
    assert.equal(fetchCalled, false, 'chatStream must not hit the network when direct-mode resolution fails');
  } finally {
    ctx.restore();
  }
});
...
test('proxy mode (back-compat default) — unknown model still routes to the proxy base URL', async () => {
  ...
  try {
    ...
    const client = new AgentClient('test-proxy-key', 'https://example-proxy.local/v1', false, 'proxy');
    const result = await client.chat(
      [{ role: 'user', content: 'hi' }],
      'totally-unknown-model-xyz-9999',
    );
    assert.equal(result.content, 'ok');
    assert.equal(seenUrls[0], 'https://example-proxy.local/v1/chat/completions');
  } finally {
    ctx.restore();
  }
});
