...
import { AtomicStagingManager, PreCommitHookRejectedError } from '../runtime/staging.js';
...
test('check — warn mode reports diagnostics but enforce() does not throw', async () => {
  const provider: LspDiagnosticsProvider = async () => [mkDiag({ severity: 'error' })];
...
test('check + enforce — block mode does not throw when only warnings exist', async () => {
  const provider: LspDiagnosticsProvider = async () => [mkDiag({ severity: 'warning' })];
  const gate = new LspPreSaveGate({ mode: 'block', provider });
  const entry = {
    id: 'x',
    targetPath: '/tmp/x',
...
test('check — provider timeout is treated as a pass-through', async () => {
  ...
  const gate = new LspPreSaveGate({ mode: 'block', provider: slow, timeoutMs: 30 });
  const result = await gate.check({
    id: 'x',
    targetPath: '/tmp/x',
    pendingPath: '/tmp/x',
