...
test("shouldDeferRead: small file but near-full conversation defers (uses both signals)", () => {
  ...
  const estimate = {
    bytes: 5000,
    projectedTokens: Math.floor(limit * 0.05),
    exact: true,
  }; // 5% more
...
test("shouldDeferRead: budgetPct of 1.0 effectively disables the gate", () => {
  ...
  const decisionDefault = shouldDeferRead(
    fittingEstimate,
    0,
    "gpt-4",
    DEFAULT_PREDICTIVE_BUDGET_PCT,
...
test("shouldDeferRead: invalid budgetPct (NaN, 0, negative) falls back to default", () => {
  ...
  const fitting = {
    bytes: 100,
    projectedTokens: Math.floor(limit * 0.5),
    exact: true,
  };
...
test("formatPredictiveGateDirective: contains the Context-Budget Guard marker and routes to alternatives", () => {
  ...
  const decision = {
    defer: true,
    reason: "predicted X",
    projectedTotal: 100_000,
    hardCap: 80_000,
  };
  const directive = formatPredictiveGateDirective(
    "src/huge.ts",
    estimate,
    decision,
  );
...
test("soak: sequential reads flip from allow→defer at a deterministic threshold", async () => {
  ...
  try {
    ...
    const steps = [0.3, 0.5, 0.7, 0.8, 0.85, 0.9, 0.95];
    let firstDeferIdx = -1;
    for (let i = 0; i < steps.length; i++) {
      ...
      if (firstDeferIdx >= 0 && i > firstDeferIdx) {
        ...
        assert.ok(
          deferred,
          `step ${i} (pct=${steps[i]}) regressed to allow after a defer`,
        );
      }
    }
    ...
    assert.ok(
      firstDeferIdx >= 0,
      "predictive gate never tipped across the 30–95% sweep",
    );
    assert.ok(
      steps[firstDeferIdx]! >= 0.8,
      `expected tip at ≥80% pressure, got ${steps[firstDeferIdx]}`,
    );
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
