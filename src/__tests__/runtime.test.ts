...
test("classifyComplexityHeuristic detects simple and complex tasks", () => {
  ...
  assert.equal(
    classifyComplexityHeuristic("explain how this database works").complexity,
    "simple",
  );

  // Single file edits are simple
  assert.equal(
    classifyComplexityHeuristic("add input validation in src/auth.ts")
      .complexity,
    "simple",
  );
  ...
  assert.equal(
    classifyComplexityHeuristic(
      "modify src/auth.ts and src/db.ts to support dynamic sessions",
    ).complexity,
    "complex",
  );
  ...
  assert.equal(
    classifyComplexityHeuristic(
      "refactor the entire codebase for parallel orchestration",
    ).complexity,
    "complex",
  );
});
...
test("Git-Ops branch creation and commit operations", () => {
  ...
  const commitLog = execSync("git log -1 --format=%s", {
    cwd: root,
    encoding: "utf-8",
  }).trim();
...
test("Orchestrator validateAndSortDAG detects cycles and orders tasks", () => {
  ...
  assert.throws(
    () => orchestrator.validateAndSortDAG(cycleTasks),
    /Circular dependency/,
  );
});
...
test("AgentPool executes hardcoded DAG concurrently respecting dependencies", async () => {
  ...
  assert.equal(
    dag.subtasks.every((s) => s.status === "completed"),
    true,
  );
...
test("WorkspaceGuard symlink escape protection resolves real paths", () => {
  ...
  const tempDirOutside = fs.mkdtempSync(
    path.join(os.tmpdir(), "fixo-symlink-outside-"),
  );
