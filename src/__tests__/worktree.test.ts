...
test('parseWorktreeAnnotations: extracts create with branch and base', () => {
  const r = parseWorktreeAnnotations('foo [worktree:create branch=feat-x base=main] bar');
...
test('parseWorktreeAnnotations: extracts create with quoted path', () => {
  const r = parseWorktreeAnnotations('[worktree:create branch=foo path="/tmp/my wt"]');
  assert.equal(r.length, 1);
...
test('parseWorktreeAnnotations: extracts remove with deleteBranch flag', () => {
  const r = parseWorktreeAnnotations('[worktree:remove path=/tmp/wt deleteBranch=true branch=feat-x]');
  assert.equal(r.length, 1);
...
test('parseWorktreeAnnotations: extracts multiple annotations in order', () => {
  const text = '[worktree:create branch=a] ... [worktree:create branch=b] ... [worktree:merge branch=a]';
...
test('applyWorktreeAnnotations: full create→merge chain on a real repo', () => {
  ...
  try {
    makeRepo(cwd);
    const annotations = [
      { op: 'create' as const, branch: 'feat-chain' },
    ];
