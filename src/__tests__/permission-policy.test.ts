...
test("parseRulePattern: Tool(arg-glob)", () => {
  assert.deepEqual(parseRulePattern("Bash(npm test:*)"), {
    tool: "Bash",
    argPattern: "npm test:*",
  });
  assert.deepEqual(parseRulePattern("Edit(src/**/*.ts)"), {
    tool: "Edit",
    argPattern: "src/**/*.ts",
  });
});
...
test("parseRulePattern: tool-only pattern", () => {
  assert.deepEqual(parseRulePattern("Bash"), {
    tool: "Bash",
    argPattern: null,
  });
});
...
test("buildArgString: falls back to joined arg values", () => {
  assert.equal(
    buildArgString("random_tool", { a: 1, b: "two", c: true }),
    "1 two true",
  );
});
...
test("matchPermission: first-match-wins", () => {
  ...
  assert.equal(
    m?.decision,
    "ask",
    "first rule wins, even though second would allow",
  );
});
...
test("checkPermission: rule hit short-circuits", () => {
  ...
  try {
    savePermissionsFile(cwd, {
      version: 1,
      rules: [
        { pattern: "Bash(npm test*)", decision: "allow", reason: "safe test" },
      ],
    });
...
test("checkPermission: deny rule wins over default-ask for new tools", () => {
  ...
  try {
    savePermissionsFile(cwd, {
      version: 1,
      rules: [
        { pattern: "str_replace(*)", decision: "deny", reason: "too risky" },
      ],
    });
    const r = checkPermission(
      "str_replace",
...
test("checkPermission: legacy tool with no rule → fallback-policy", () => {
  ...
  try {
    ...
    assert.equal(
      r.decision,
      "allow",
      "read actions are allowed by decidePolicy",
    );
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
