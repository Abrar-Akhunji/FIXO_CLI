...
test("Sprint 2 Heuristics and Intent Classification", async (t) => {
  await t.test(
    "createStructuredPlan ignores domains and version numbers",
    () => {
      const plan = createStructuredPlan(
        "fix bugs in index.ts and check google.com or v2.0 package",
      );
  ...
  await t.test("evaluateInputIntent strictly matches word boundaries", () => {
    ...
    assert.equal(
      evaluateInputIntent("refactor the validation list component"),
      "MUTATION",
    );
    ...
    assert.equal(
      evaluateInputIntent("playlist manager implementation"),
      "MUTATION",
    );
    assert.equal(
      evaluateInputIntent("find vulnerabilities in codebase and list them"),
      "MUTATION",
    );
...
test("Sprint 2 Conversation Pruning and Centralized Defaults", async (t) => {
  ...
  await t.test(
    "ConversationManager turn-aware pruning retains complete turns",
    () => {
