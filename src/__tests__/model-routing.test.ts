...
test("modelRouting.fast — required_capabilities: ['fast'] substitutes the fast-tier model on the wire", async () => {
  ...
  try {
    ...
    await client.chat([{ role: "user", content: "classify this" }], "gpt-4o", {
    ...
    assert.equal(
      sent.model,
      "gpt-4o-mini",
      "fast tier must substitute the request-body model",
    );
  } finally {
    ctx.restore();
  }
...
test("modelRouting.heavy — required_capabilities: ['heavy'] substitutes the heavy-tier model on the wire", async () => {
  ...
  try {
    ...
    await client.chat([{ role: "user", content: "tough refactor" }], "gpt-4o", {
    ...
    assert.equal(
      sent.model,
      "o3",
      "heavy tier must substitute the request-body model",
    );
  } finally {
    ctx.restore();
  }
...
test("modelRouting — no capability tag means the caller model passes through unchanged", async () => {
  ...
  try {
    ...
    assert.equal(
      sent.model,
      "gpt-4o",
      "no capability tag must NOT substitute the model",
    );
  } finally {
    ctx.restore();
  }
...
test("modelRouting — when fast tier is not configured, fast-tagged calls fall through to the caller model", async () => {
  ...
  try {
    ...
    await client.chat([{ role: "user", content: "classify" }], "gpt-4o", {
      required_capabilities: ["fast"],
    });
    ...
    assert.equal(
      sent.model,
      "gpt-4o",
      "unset fast tier must fall through to caller model",
    );
  } finally {
    ctx.restore();
  }
...
test("modelRouting — back-compat: undefined modelRouting (older config) routes unchanged", async () => {
  ...
  try {
    ...
    const client = new AgentClient("", undefined, false, "direct");
    await client.chat([{ role: "user", content: "whatever" }], "gpt-4o", {
      required_capabilities: ["fast"],
    });
    ...
    assert.equal(
      sent.model,
      "gpt-4o",
      "absent modelRouting must NOT modify the model",
    );
  } finally {
    ctx.restore();
  }
