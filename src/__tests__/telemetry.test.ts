...
test('recordTelemetry — appends a single NDJSON line', () => {
  withTempHome(() => {
    const event = telemetry.toolCall({ tool: 'read_file', status: 'completed' });
...
test('recordTelemetry — multiple events are appended in order', () => {
  withTempHome(() => {
    recordTelemetry(telemetry.retry({ fn: 'chat', attempt: 1, delayMs: 100, error: 'timeout' }));
    recordTelemetry(telemetry.cooldown({ providerId: 'groq', status: 429, cooldownMs: 30_000, reason: 'rate limit' }));
...
test('readRecentEvents — honours the limit', () => {
  withTempHome(() => {
    for (let i = 0; i < 5; i++) {
      recordTelemetry(telemetry.retry({ fn: 'chat', attempt: i, delayMs: 100, error: 'e' }));
    }
...
test('recordTelemetry — returns false when the file cannot be written', () => {
  ...
  // cannot be created.
  const original = process.env.HOME;
  process.env.HOME = '/this/path/definitely/does/not/exist/and/cannot/be/created';
  try {
    const ok = recordTelemetry(telemetry.toolCall({ tool: 'x', status: 'completed' }));
...
test('diagnoseFailures — flags a retry storm (>=3 retries)', () => {
  withTempHome(() => {
    for (let i = 0; i < 4; i++) {
      recordTelemetry(telemetry.retry({ fn: 'chat', attempt: i, delayMs: 100, error: 'timeout' }));
    }
...
test('diagnoseFailures — flags a provider cooldown', () => {
  withTempHome(() => {
    recordTelemetry(
      telemetry.cooldown({ providerId: 'groq', status: 429, cooldownMs: 30_000, reason: 'rate limit' }),
    );
    const hints = diagnoseFailures();
...
test('diagnoseFailures — flags stream resume exhaustion as error severity', () => {
  withTempHome(() => {
    recordTelemetry(
      telemetry.streamResume({ resumeAttempt: 3, partialTokens: 0, ok: false, reason: 'exhausted' }),
    );
    const hints = diagnoseFailures();
    const exhausted = hints.find((h) => h.severity === 'error');
    assert.ok(exhausted, `expected an error-severity hint, got ${JSON.stringify(hints)}`);
  });
});
...
test('diagnoseFailures — flags compaction as info', () => {
  withTempHome(() => {
    ...
    const hints = diagnoseFailures();
    assert.ok(hints.some((h) => h.severity === 'info' && h.summary.includes('Context')));
  });
});
...
test('diagnoseFailures — clusters >=3 failures of the same tool', () => {
  withTempHome(() => {
    for (let i = 0; i < 3; i++) {
      recordTelemetry(telemetry.toolCall({ tool: 'shell', status: 'failed', error: 'EACCES' }));
    }
    const hints = diagnoseFailures();
    const toolHint = hints.find((h) => h.summary.includes('shell'));
    assert.ok(toolHint, `expected a tool-cluster hint, got ${JSON.stringify(hints)}`);
  });
});
...
test('diagnoseFailures — ignores events outside the window', () => {
  withTempHome(() => {
    ...
    for (let i = 0; i < 5; i++) {
      const event = telemetry.retry({ fn: 'chat', attempt: i, delayMs: 100, error: 'e' });
      // Backdate by 2 hours.
      const backdated: TelemetryEvent = { ...event, ts: new Date(Date.now() - 2 * 60 * 60_000).toISOString() };
...
test('recordTelemetry — rotates when file exceeds 1 MiB', () => {
  withTempHome(() => {
    ...
    recordTelemetry(telemetry.toolCall({ tool: 'after-rotate', status: 'completed' }));
