...
test("inlineFormat: backticks → PURPLE, bold → BOLD+SNOW, italic → italic+SNOW3", () => {
  const out = __internal.inlineFormat(
    "use `npm test` and **stop** when _done_",
  );
...
test("renderMarkdown: fenced code emits a framed box with lang label", () => {
  ...
  assert.ok(
    sideMatches.length >= 4,
    `expected at least 4 │ (2 lines × 2 sides) got ${sideMatches.length}`,
  );
});
...
test("renderMarkdown: blank line preserved between paragraphs", () => {
  ...
  const idxSecond = lines.findIndex((l) =>
    strip(l).includes("second paragraph"),
  );
...
test("renderMarkdown: wide table proportionally shrinks to fit terminal width", () => {
  ...
  for (const line of lines) {
    assert.ok(
      line.length <= 82,
      `line exceeded terminal width: ${line.length} > 82\n${line}`,
    );
  }
...
test("renderMarkdown: paragraph longer than viewport hard-wraps without overflow", () => {
  ...
  for (const line of lines) {
    assert.ok(
      line.length <= 62,
      `paragraph line exceeded width: ${line.length}\n${line}`,
    );
  }
});
