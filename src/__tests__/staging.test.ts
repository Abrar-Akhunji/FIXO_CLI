...
test('commit — pre-commit hook rejection preserves the target', async () => {
  await withTempCwd(async (cwd) => {
    ...
    await assert.rejects(
      () => mgr.commit(entry.id),
      (err: unknown) =>
        err instanceof PreCommitHookRejectedError && /LSP gate rejected/.test(String(err)),
    );
    // Target preserved.
    assert.equal(fs.readFileSync(target, 'utf-8'), 'original');
    // Staged write was discarded.
    assert.throws(() => mgr.readEntry(entry.id), (err: unknown) => err instanceof StagedWriteNotFoundError);
  });
});
...
test('commit — missing staged write throws StagedWriteNotFoundError', async () => {
  await withTempCwd(async (cwd) => {
    const mgr = new AtomicStagingManager(cwd, 'r1');
    await assert.rejects(
      () => mgr.commit('nonexistent-id-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'),
...
test('gc — removes entries older than ttlMs', async () => {
  await withTempCwd((cwd) => {
    ...
    const meta = JSON.parse(fs.readFileSync(metaPath, 'utf-8')) as { createdAt: number };
