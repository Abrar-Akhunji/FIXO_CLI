...
const baseSafety: SafetyConfig = {
  ...
  lspPreSave: 'warn',
  loopTrap: { triggerCount: 3, hardAbortCount: 6, toolResultTailBytes: 1024, maxHistory: 64, enabled: true },
  ...
  toolCalls: { softLimit: 50, hardLimit: 100, autoExtend: true, investigationMultiplier: 3 },
  sandboxMode: 'guard',
  autoVerify: true,
...
test('decideAutoVerify — skipped outside BUILD mode (PLAN/EXPLORE/SCOUT)', () => {
  for (const mode of ['PLAN', 'EXPLORE', 'SCOUT'] as const) {
    ...
    assert.deepEqual(d, { run: false, reason: 'wrong-mode' }, `expected skip for mode=${mode}`);
  }
});
...
test('classifyVerifyOutput — Status: 0 maps to passing', () => {
  assert.equal(classifyVerifyOutput('Status: 0\nOutput:\nAll 12 tests passed'), 'passing');
});

test('classifyVerifyOutput — non-zero status maps to failing', () => {
  assert.equal(classifyVerifyOutput('Status: 1\nRelevant output:\nFAIL src/foo.test.ts'), 'failing');
});

test('classifyVerifyOutput — "No test or build command detected" maps to no-command', () => {
  assert.equal(classifyVerifyOutput('No test or build command detected.'), 'no-command');
});
...
test('classifyVerifyOutput — empty / unrecognized output defaults to failing (conservative)', () => {
  ...
  assert.equal(classifyVerifyOutput('weird output that doesnt match any known shape'), 'failing');
});
...
test('buildRepairMessage — includes the test output verbatim and a fix directive', () => {
  ...
  assert.ok(msg.includes(out), 'repair message must include the test output verbatim');
});
