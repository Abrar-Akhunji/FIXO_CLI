...
test('scrubForLlm — redacts Anthropic keys', () => {
  const text = 'Authorization: sk-ant-api03-abcdefghijklmnopqrstuvwxyz1234567890ABCD';
...
test('scrubForLlm — strips ANSI before scanning', () => {
  ...
  const text = '\x1b[31msk-ant-api03-abcdefghijklmnopqrstuvwxyz1234567890ABCD\x1b[0m';
  const out = scrubForLlm(text);
  assert.equal(out.includes('sk-ant-api03-'), false);
});

test('scrubForLlm — leaves ordinary prose untouched', () => {
  const text = 'The quick brown fox jumps over the lazy dog. 1234567890 is just a number.';
...
test('scrubForLlm strips colour before scrubbing secrets', () => {
  ...
  const input = '\x1b[32mAuthorization: Bearer abcdefghijklmnopqrstuvwxyz0123456789\x1b[0m';
