...
test('glob_files — matches TypeScript sources under a nested directory', async () => {
  await withTempCwd(async (cwd) => {
    ...
    fs.writeFileSync(path.join(cwd, 'src/a.ts'), 'export const a = 1;', 'utf-8');
    fs.writeFileSync(path.join(cwd, 'src/nested/b.ts'), 'export const b = 2;', 'utf-8');
...
test('glob_files — default skip set prunes node_modules, .git, dist, .fixo, .fixocli', async () => {
  await withTempCwd(async (cwd) => {
    ...
    fs.writeFileSync(path.join(cwd, '.git/HEAD'), 'ref: refs/heads/main', 'utf-8');
    ...
    fs.writeFileSync(path.join(cwd, '.fixo/staging/run1/pending'), 'x', 'utf-8');
    ...
    assert.equal(joined.includes('node_modules'), false, 'node_modules must be skipped');
    ...
    assert.equal(joined.includes('.fixocli/'), false, '.fixocli must be skipped');
    assert.ok(
...
test('glob_files — explicit cwd that escapes the workspace is rejected', async () => {
  await withTempCwd(async (cwd) => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'fixo-glob-outside-'));
    try {
      const args: GlobArgs = {
        pattern: '**/*.ts',
...
test('glob_files — overflow truncates results and reports total + truncated flag', async () => {
  await withTempCwd(async (cwd) => {
    ...
    for (let i = 0; i < 50; i++) {
      fs.writeFileSync(path.join(cwd, `file-${i.toString().padStart(2, '0')}.txt`), `${i}`, 'utf-8');
    }
...
test('glob_files — hard cap clamps maxResults to 5000', async () => {
  await withTempCwd(async (cwd) => {
    ...
    const parsed = JSON.parse(result) as { matches: string[]; truncated: boolean };
