...
test('saveHooksFile + loadHooksFile roundtrip preserves the spec', () => {
  ...
  try {
    const file: HooksFile = {
      version: 1,
      hooks: [
        { id: 'demo-pre', event: 'PreToolUse', command: 'node', args: ['-e', 'process.exit(0)'] },
      ],
    };
...
test('fireHooks PreToolUse with allow hook passes through', () => {
  ...
  try {
    saveHooksFile(cwd, {
      version: 1,
      hooks: [
        { id: 'silent', event: 'PreToolUse', command: 'node', args: ['-e', 'process.exit(0)'] },
      ],
    });
    const r = fireHooks(cwd, 'PreToolUse', {
      tool: 'write_file',
      ...
      hooks: [
        {
          ...
          event: 'PreToolUse',
          command: 'node',
          args: ['-e', 'process.stdout.write(JSON.stringify({decision:"deny",reason:"no touch"}))'],
        },
      ],
    });
    const r = fireHooks(cwd, 'PreToolUse', {
...
test('fireHooks first deny in chain short-circuits the rest', () => {
  ...
  try {
      ...
      hooks: [
        {
          ...
          event: 'PreToolUse',
          command: 'node',
          args: ['-e', 'process.stdout.write(JSON.stringify({decision:"deny",reason:"first"}))'],
        },
        {
          id: 'later',
          event: 'PreToolUse',
    ...
    assert.equal(fs.existsSync(laterRan), false, 'later hook should not have run');
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
});
...
test('fireHooks only runs hooks for the requested event', () => {
  ...
  try {
      ...
      hooks: [
        {
          ...
          event: 'PostToolUse',
          command: 'node',
          args: ['-e', 'process.stdout.write(JSON.stringify({decision:"deny"}))'],
        },
      ],
    });
    const r = fireHooks(cwd, 'PreToolUse', {
    ...
    assert.equal(r.fired, false, 'PostToolUse hook should not fire on PreToolUse event');
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
});
...
test('applyModifiedArgs rejects modifiedArgs with out-of-workspace absolute path', () => {
  ...
  try {
    ...
    assert.deepEqual(r.args, { path: 'a.ts' }, 'should fall back to original args');
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
});
...
test('executeTool: PreToolUse hook with deny decision blocks the call (PRD §5.7)', async () => {
  ...
  try {
    ...
    const event = await executeTool('write_file', { path: 'a.ts', content: 'x' }, cwd);
