...
import {
  AgentPool,
  computePartialCommitPlan,
  findInFlightConflict,
} from "../agent/agent-pool.js";
import {
...
test("AgentPool default subtaskBudget is 100", () => {
  const defaults = getAgentPoolConfig(getDefaultConfig());
  assert.equal(defaults.subtaskBudget, 100);
});
...
test("getAgentConfig falls back to defaults for configs predating the namespace", () => {
  ...
  assert.equal(resolved.pool.subtaskBudget, 100);
...
test("computePartialCommitPlan: flag off → never takes partial path", () => {
  ...
  const plan = computePartialCommitPlan(subtasks, {
...
test("computePartialCommitPlan: flag on + mixed → partial", () => {
  ...
  const subtasks: Subtask[] = [
    makeSubtask({
      id: "animations",
      status: "completed",
      touchedFiles: ["/ws/style.css", "/ws/script.js"],
    }),
    makeSubtask({
      id: "palette",
      status: "failed",
      touchedFiles: ["/ws/palette.txt"],
    }),
  ...
  assert.deepEqual(plan.successFiles.sort(), [
    "/ws/script.js",
    "/ws/style.css",
  ]);
...
test("computePartialCommitPlan: file touched by both → attributed to success", () => {
  ...
  const subtasks: Subtask[] = [
    makeSubtask({
      id: "a",
      status: "completed",
      touchedFiles: ["/ws/shared.css"],
    }),
    makeSubtask({
      id: "b",
      status: "failed",
      touchedFiles: ["/ws/shared.css", "/ws/onlyB.css"],
    }),
  ];
  const plan = computePartialCommitPlan(subtasks, {
    preservePartialOnFailure: true,
...
test("findInFlightConflict: missing files + flag on → defers", () => {
  ...
  assert.equal(
    findInFlightConflict(candidate, inFlight, { serializeMissingFiles: true }),
    "peer",
  );
});
...
test("findInFlightConflict: missing files + flag off → allows", () => {
  ...
  assert.equal(
    findInFlightConflict(candidate, inFlight, { serializeMissingFiles: false }),
    null,
  );
});
