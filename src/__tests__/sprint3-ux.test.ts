...
test('Sprint 3 UX Polish Tests', async (t) => {
  ...
  await t.test('SingleAgent askPermission "Yes to all" sets allowAll to true', async () => {
    const agent = new SingleAgent();
  ...
  await t.test('WorkerAgent askPermission allowWithoutPrompt and allowAll flags', async () => {
    const worker = new WorkerAgent();
    // 1. context.yes should bypass
    const res1 = await (worker as any).askPermission('write_file', { path: 'test.txt' }, undefined, { yes: true });
    assert.equal(res1, true);

    // 2. allowAll should bypass
    (worker as any).allowAll = true;
    const res2 = await (worker as any).askPermission('write_file', { path: 'test.txt' });
    assert.equal(res2, true);
  });

  await t.test('WorkerAgent askPermission "all" input sets allowAll to true', async () => {
    const worker = new WorkerAgent();
    ...
    const promise = (worker as any).askPermission('write_file', { path: 'test.txt' }, mockRl as any);
