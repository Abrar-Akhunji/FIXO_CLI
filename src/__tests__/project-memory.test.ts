...
test('SQLite facts retrieval and TF-IDF fallback', async () => {
  ...
  const relevant = await retrieveRelevantFacts(tempDir, 'node database sqlite', undefined, 1);
...
test('Plugin loader allowlist validation', async () => {
  ...
  await initializePlugins(tempDir, { plugins: [pluginFile], trustedPlugins: [] });
  assert.ok(!loadedPlugins.some(p => p.path === pluginFile), 'Plugin should not load if not in trustedPlugins');
  ...
  await initializePlugins(tempDir, { plugins: [pluginFile], trustedPlugins: [pluginFile] });
  assert.ok(loadedPlugins.some(p => p.path === pluginFile), 'Plugin should load if trusted and approved');
});
