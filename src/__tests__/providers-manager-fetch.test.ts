...
test("fetchRemoteModels — live success persists cache and returns live source", async () => {
  ...
  try {
    ...
    assert.ok(
      capture.url?.endsWith("/models"),
      `unexpected url: ${capture.url}`,
    );
    assert.equal(
...
test("fetchRemoteModels — zen provider sends HTTP-Referer + X-Title headers", async () => {
  ...
  try {
    ...
    assert.equal(
      capture.headers?.["Authorization"],
      "Bearer zen-test-key-1234567890",
    );
  } finally {
    ctx.restore();
  }
...
test("verifyKeyAndFetchModels — success fetches models and caches them", async () => {
  ...
  try {
    ...
    const models = await ProvidersManager.verifyKeyAndFetchModels(
      "openai",
      "sk-test-key-555",
    );
      ...
      (err: any) => {
        assert.ok(
          err.message.includes(
            "API returned status 401 Unauthorized: Invalid API Key",
          ),
...
test("verifyKeyAndFetchModels — network error throws descriptive error", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (err: any) => {
        assert.ok(
          err.message.includes("Network error or timeout connecting to OpenAI"),
          `unexpected error message: ${err.message}`,
        );
        return true;
      },
