...
import { extractWriteTargets, isCommandSafe } from "../agent/command-parser.js";
...
test("extractWriteTargets — `cat > file` redirect", () => {
  const ts = extractWriteTargets(
    "cat > src/components/BookingCTA.jsx << 'EOF'\nhi\nEOF",
  );
  assert.ok(
    ts.some(
      (t) =>
        t.kind === "redirect" && t.path === "src/components/BookingCTA.jsx",
    ),
  );
});
...
test("extractWriteTargets — `sed -i` in-place", () => {
  ...
  assert.ok(
    ts.some((t) => t.kind === "sed-in-place" && t.path === "src/index.ts"),
  );
});
