...
function toolDelta(): StreamChunk {
  return {
    ...
    tool_call: { index: 0, function: { arguments: '{"p' } },
  };
}
...
test("reconstructPartialResponse handles thinking mixed with content", () => {
  ...
  assert.equal(
    reconstructPartialResponse(chunks),
    "Reasoning: first more reasoning second",
  );
});
...
test("isMidStreamResumable returns true for network errors", () => {
  for (const msg of [
    ...
    assert.equal(
      isMidStreamResumable(err),
      true,
      `expected ${msg} to be resumable`,
    );
  }
});
...
test("isMidStreamResumable returns true for 5xx HttpError and 408/425", () => {
  for (const status of [408, 425, 500, 502, 503, 504]) {
    assert.equal(
      isMidStreamResumable(new HttpError(status, "boom")),
      true,
      `status ${status}`,
    );
  }
