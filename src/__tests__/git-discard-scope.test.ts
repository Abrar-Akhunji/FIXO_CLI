...
test("discardChangesIn — absolute paths are normalised to workspace-relative", () => {
  ...
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(ctx.cwd, "foo.txt"), "utf-8"),
      "committed\n",
    );
  } finally {
    ctx.cleanup();
...
test("discardChangesIn — empty list is a true no-op (no files touched, no errors thrown)", () => {
  ...
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(ctx.cwd, "bar-untracked.txt"), "utf-8"),
      "untracked\n",
    );
  } finally {
    ctx.cleanup();
...
test("discardChangesIn — silently skips files that were never dirty", () => {
  ...
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(ctx.cwd, "clean.txt"), "utf-8"),
      "committed\n",
    );
  } finally {
    ctx.cleanup();
...
test("forceDiscardAllUncommittedChanges — refuses without the iAmCertain flag", () => {
  ...
  try {
    ...
    (
      gm as unknown as {
        forceDiscardAllUncommittedChanges: (opts: object) => void;
      }
