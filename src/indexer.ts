...
const CODE_EXTENSIONS = new Set(['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.json', '.md']);

function resolveInternalImport(cwd: string, sourceFile: string, importStr: string): string | null {
  ...
  if (['.js', '.jsx', '.mjs', '.cjs'].includes(extname)) {
    const baseWithoutExt = path.resolve(sourceDir, importStr.slice(0, -extname.length));
...
export function findCodebaseDependencies(cwd: string, targetPath: string): string {
  ...
  const importance = file.importance !== undefined ? (file.importance * 100).toFixed(2) : 'N/A';
...
function listFiles(root: string): string[] {
  ...
  const walk = (dir: string) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (fs.statSync(full).size <= 300_000) result.push(path.relative(root, full));
    }
  };
  walk(root);
  return result;
