...
export function recordModelOutcome(cwd: string, outcome: Omit<ModelOutcome, 'timestamp'>): void {
...
export function summarizeModelOutcomes(cwd: string): string {
  ...
  const byModel = new Map<string, { total: number; success: number; latency: number; verified: number }>();
