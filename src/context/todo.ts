...
function normaliseItems(raw: unknown): TodoItem[] {
  ...
  for (const r of raw) {
    ...
    const id = typeof obj.id === 'string' && obj.id.length > 0 ? obj.id : randomUUID();
    ...
    const blockedBy = typeof obj.blockedBy === 'string' ? obj.blockedBy : undefined;
...
export function addItem(list: TodoList, input: AddItemInput, now: number = Date.now()): TodoList {
...
export function setItemStatus(list: TodoList, input: SetStatusInput, now: number = Date.now()): TodoList {
  const items = list.items.map((it) =>
    it.id === input.id
      ? { ...it, status: input.status, updatedAt: now }
      : it,
  );
...
export function removeItem(list: TodoList, input: RemoveItemInput, now: number = Date.now()): TodoList {
  return { ...list, items: list.items.filter((it) => it.id !== input.id), updatedAt: now };
}

export function clearDoneItems(list: TodoList, now: number = Date.now()): TodoList {
  return {
    ...list,
    items: list.items.filter((it) => it.status !== 'done' && it.status !== 'cancelled'),
    updatedAt: now,
  };
}
...
export function summariseTodoList(list: TodoList): string {
  ...
  const open = list.items.filter((it) => it.status === 'pending' || it.status === 'in_progress');
...
export function renderTodoList(list: TodoList): string {
  ...
  const open = list.items.filter((it) => it.status === 'pending' || it.status === 'in_progress');
  const done = list.items.filter((it) => it.status === 'done' || it.status === 'cancelled');
  const orderFn = (a: TodoItem, b: TodoItem) => a.createdAt - b.createdAt;
  open.sort(orderFn);
  done.sort(orderFn);

  const renderItem = (it: TodoItem): string => {
    const tag = it.status === 'in_progress' ? '[~]' : it.status === 'done' ? '[x]' : it.status === 'cancelled' ? '[-]' : '[ ]';
